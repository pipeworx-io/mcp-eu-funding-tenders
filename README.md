...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/eu_search_calls \
  -H 'Content-Type: application/json' \
  -d '{"query":"battery recycling","status":"open","limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/eu_search_calls`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "eu-funding-tenders": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-eu-funding-tenders"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-eu-funding-tenders
```
